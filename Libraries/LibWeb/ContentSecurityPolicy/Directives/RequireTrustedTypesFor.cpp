...
GC_DEFINE_ALLOCATOR(RequireTrustedTypesFor);

RequireTrustedTypesFor::RequireTrustedTypesFor(String name, Vector<String> value)
    : Directive(move(name), move(value))
{
}

// https://www.w3.org/TR/trusted-types/#require-trusted-types-for-pre-navigation-check
Directive::Result RequireTrustedTypesFor::pre_navigation_check(GC::Ref<Fetch::Infrastructure::Request> request, NavigationType, GC::Ref<Policy const>) const
{
    ...
    auto const encoded_script_source = MUST(url_string.substring_from_byte_offset_with_shared_superstring("javascript:"sv.length()));
    ...
    //    sink:
    //      "Location href":
    auto converted_script_source = TrustedTypes::process_value_with_a_default_policy(
        TrustedTypes::TrustedTypeName::TrustedScript,
        request->client()->global_object(),
        Utf16String::from_utf8(encoded_script_source),
        TrustedTypes::InjectionSink::Locationhref);
    ...
    if (converted_script_source.is_error() || !converted_script_source.value().is_object())
        return Result::Blocked;

    auto const converted_script_source_value = as_if<TrustedTypes::TrustedScript>(converted_script_source.value().as_object());
...
bool does_sink_require_trusted_types(JS::Object& global, String sink_group, IncludeReportOnlyPolicies include_report_only_policies)
{
    ...
    for (auto const policy : PolicyList::from_object(global)->policies()) {
        ...
        if (directive->value().find_if([&sink_group](auto const& directive_value) { return directive_value.equals_ignoring_ascii_case(sink_group); }).is_end())
            continue;

        // 4. Let enforced be true if policy’s disposition is "enforce", and false otherwise.
        auto const enforced = policy->disposition() == Policy::Disposition::Enforce;
...
Directive::Result should_sink_type_mismatch_violation_be_blocked_by_content_security_policy(JS::Object& global, TrustedTypes::InjectionSink sink, String sink_group, Utf16String source)
{
    auto& realm = *global.vm().current_realm();

    // 1. Let result be "Allowed".
    auto result = Directive::Result::Allowed;
    ...
    for (auto const policy : PolicyList::from_object(global)->policies()) {
        ...
        auto violation = Violation::create_a_violation_object_for_global_policy_and_directive(realm, global, policy, Names::RequireTrustedTypesFor.to_string());

        // 5. Set violation’s resource to "trusted-types-sink".
        violation->set_resource(Violation::Resource::TrustedTypesSink);
        ...
        if (policy->disposition() == Policy::Disposition::Enforce)
            result = Directive::Result::Blocked;
    }
